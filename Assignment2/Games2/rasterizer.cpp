...
{   
    ...
    auto v = Vector3f(.5f + x, .5f + y, 1.0f);
    ...
    auto ab_x_ap = ab.cross(ap)[2];
    auto bc_x_bp = bc.cross(bp)[2];
    auto ca_x_cp = ca.cross(cp)[2];

    return (ab_x_ap > 0 && bc_x_bp > 0 && ca_x_cp > 0) || (ab_x_ap < 0 && bc_x_bp < 0 && ca_x_cp < 0);
}
